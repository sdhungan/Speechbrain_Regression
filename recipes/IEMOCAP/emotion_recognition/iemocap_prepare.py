...
SAMPLERATE = 16000
# NUMBER_UTT = 5531 # total number of utterances in IEMOCAP, but this is not used in the code
...
def prepare_data(
    ...
    '''
    # The number of utterances in the given code is if we used only a set of classes (neu, hap, sad, ang)
    if sum([len(value) for value in speaker_dict.values()]) != NUMBER_UTT:
        raise ValueError(
            f"Error: Number of utterances is {sum([len(value) for value in speaker_dict.values()])}, but it should be {NUMBER_UTT}."
        )
    '''
        ...
        #):
            ...
            if emoFile[7] != "i" and utterance[2][7] == "s":
                improvisedUtteranceList.append(
                    [path, valance, arousal, utterance[2][18]]
                )
            else:
                improvisedUtteranceList.append(
                    [path, valance, arousal, utterance[2][15]]
                )
