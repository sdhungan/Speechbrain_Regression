...
class CustomEncoderWav2vec2Classifier(Pretrained):
    ...
    def classify_file(self, path):
        ...
        prediction = self.mods.output_mlp(outputs).squeeze(1)
        return prediction
    ...
    def find_regression_file(self, path):
        """Performs regression on the given audio file.

        Arguments
        ---------
        path : str
            Path to audio file to perform regression on.

        Returns
        -------
        regression_result
            The regression result for the given audio file.
        """
        waveform = self.load_audio(path)
        # Fake a batch:
        batch = waveform.unsqueeze(0)
        rel_length = torch.tensor([1.0])
        outputs = self.encode_batch(batch, rel_length)
        regression_result = self.mods.output_mlp(outputs).squeeze(1)
